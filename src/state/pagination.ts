...
export function selectPageEnd(state: PaginationState, filteredCount: number): number {
    return clamp(state.pageStart + state.pageCount, 0, filteredCount);
}

export function selectCurrentPage(state: PaginationState, sortedMaps: BeatmapInfo[]): BeatmapInfo[] {
    const { pageStart } = state;
    const pageEnd = selectPageEnd(state, sortedMaps.length);
    return sortedMaps.slice(pageStart, pageEnd);
}

export function handlePaginationAction(state: PaginationState, action: PaginationAction): PaginationState {
    switch (action.type) {
