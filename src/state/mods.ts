...
export interface ModsState {
    ...
    fetchMods: Partial<
        Record<ModCombination, "needed" | "fetching" | "fetched">
    >;
}
...
export function handleModsAction(
    state: ModsState,
    action: ModsAction
): ModsState {
    switch (action.type) {
        case "setCurrentMods": {
            return {
                ...
                fetchMods: state.fetchMods[action.value]
                    ? state.fetchMods
                    : {
                          ...state.fetchMods,
                          [action.value]: "needed",
        ...
        case "addNeededMods": {
            if (
                action.neededMods.find((mods) => !state.fetchMods[mods]) ==
                undefined
            ) {
                return state;
            }
