...
const PageNavigation = (props: {
    ...
    const handleClickPrev = () => {
        dispatch({
            type: "setPageStart",
            value: Math.max(0, pageStart - pageCount),
            filteredCount,
        });
    };
        ...
        <nav>
            <ul className="pagination justify-content-center">
                ...
                <li
                    className={classNames(
                        "page-item",
                        pageEnd === filteredCount && "disabled"
                    )}
                >
...
export const App = () => {
    ...
    const [songPreview, dispatchSongPreview] = React.useReducer(
    ...
    const unfetchedMods = new Set(
        MOD_COMBINATIONS.filter((m) => !mods.fetchMods[m])
    );
    const { querySource } = filter;

    React.useEffect(() => {
        const result = selectQueryExpression(querySource, (mods) =>
            unfetchedMods.has(mods) ? undefined : -1
        );
        ...
        for (const mods of MOD_COMBINATIONS) {
            ...
            fetch(
                ...
                .then(async (req) => {
                    if (!req.ok)
                        throw new Error(
                            `Failed to fetch ${req.url}: ${req.statusText}`
                        );
