...
const StatusFilterField = (props: {
    ...
    return <div className="col field">
        ...
        <select id={ID} className="form-control"
            ...
            onChange={e => props.dispatch({ type: 'setStatusFilter', value: parseInt(e.target.value) })}>
...
const RulesetFilterField = (props: {
    ...
    return <div className="col field">
        ...
        <select id={ID} className="form-control"
            ...
            onChange={e => props.dispatch({ type: 'setRulesetFilter', value: parseInt(e.target.value) })}>
...
const ModsSelectField = (props: {
    ...
    return <div className="col field">
        ...
        <select id={ID} className="form-control"
            ...
            onChange={e => props.dispatch({ type: 'setCurrentMods', value: parseInt(e.target.value) })}>
...
const LocalDataFilterField = (props: {
    ...
    return <div className="col field">
        ...
        <select id={ID} className="form-control"
            ...
            onChange={e => props.dispatch({ type: 'setLocalDataFilter', value: parseInt(e.target.value) })}>
...
const FiltersTab = (props: {
    ...
    return <div className="row">
        <StatusFilterField statusFilter={filter.statusFilter} dispatch={dispatch} />
        <RulesetFilterField rulesetFilter={filter.rulesetFilter} dispatch={dispatch} />
        <QueryFilterField querySource={filter.querySource} dispatch={dispatch} />
        <ModsSelectField currentMods={currentMods} dispatch={dispatch} />
        <LocalDataFilterField localDataFilter={filter.localDataFilter} dispatch={dispatch} />
        <div className="col"><p>
            <a href="#">Reset filters</a><br />
            <input style={{ width: "3em" }}
                value={pagination.pageStart}
                onChange={handlePageStartChange}
            />..{selectPageEnd(pagination, filteredCount)} of {filteredCount} maps<br />
            <input style={{ width: "3em" }}
                value={pagination.pageCount}
                onChange={handlePageCountChange} /> per page
        </p></div>
    </div>;
};

const LocalDataTab = (props: {
    ...
    return <div className="row">
        <div className="col field">
            <label htmlFor={ID} style={{ paddingRight: "1em" }}>Load osu!.db file</label>
            <input id={ID} className="form-control-file" type="file" multiple
...
const TabPane = <K extends string>(props: {
    ...
    return <div className={classNames('tab-pane', props.active(props.eventkey) && 'active')}>
        {props.children}
    </div>
};
...
const Header = (props: {
    ...
    currentMods: ModCombination,
    filteredCount: number,
    dispatch: React.Dispatch<ModsAction | DataAction | FilterAction | PaginationAction>,
}) => {
    type TabId = 'filters' | 'local_data' | 'about';

    const [activeTab, setActiveTab] = React.useState('filters' as TabId);
    const active = React.useCallback((key: TabId) => key === activeTab, [activeTab]);

    return <div className="header container-fluid">
        <ul className="nav nav-tabs">
            <Tab eventkey="filters" active={active} setActive={setActiveTab}>Filters</Tab>
            <Tab eventkey="local_data" active={active} setActive={setActiveTab}>Local data</Tab>
            <Tab eventkey="about" active={active} setActive={setActiveTab}>About</Tab>
        </ul>
